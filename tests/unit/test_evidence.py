...
def test_evidence_scorer_renormalizes_missing_sources() -> None:
    r = EvidenceRecord(gene_id="x")
    r.add_score(EvidenceSource.EXPRESSION, 1.0)
    r.add_score(EvidenceSource.RNai, 0.0)
    s = EvidenceScorer()
    w = s.weights
    # Renormalized weights over the two present sources: expression gets
    # w(expr) / (w(expr) + w(rnai)) of the total -> integrated = that ratio.
    expect = w[EvidenceSource.EXPRESSION] / (
        w[EvidenceSource.EXPRESSION] + w[EvidenceSource.RNai]
    )
    assert abs(s.integrated_score(r) - expect) < 1e-9
