...
def test_step_add_function_stream_attaches_unit_score_for_known() -> None:
    import bioforge.workflow.steps as _steps  # noqa: F401
    from bioforge.evidence.schema import EvidenceRecord, EvidenceSource
    from bioforge.workflow.registry import StepRegistry

    fn = StepRegistry.instance().get("evidence.add_function_stream")
    out = fn(records=[EvidenceRecord(gene_id="g1", gene_name="soxB")])
    assert out["records"][0].scores[EvidenceSource.FUNCTION] == 1.0
