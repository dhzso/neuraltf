...
@pytest.mark.parametrize("name,expected", [
    ...
    ("TF_fork_head", True),
    ("TF_T-box", True),
    # 2026-09-06 audit: real PlanMine corpus names the old keyword
    # classifier missed (false negatives) — all must gate-pass now.
    ("HMG_box_dom", True),          # SOX factors
    ("Paired_dom", True),           # Pax factors
    ("TF_Brachyury", True),         # T-box
    ("TCF/LEF", True),
    ("CUT_dom", True),              # CUT homeobox
    ("Iroquois_homeo", True),
    ("Homeo_prospero_dom", True),
    ("Transcription_factor_COE", True),
    ("TF_AP2", True),
    ("ARID_dom", True),
    ("MAD_homology_MH1", True),     # SMAD DNA-binding lobe
    ("DMRT/dsx/mab-3", True),
    ("Teashirt_fam", True),
    ("Nucl_hormone_rcpt_ligand-bd", True),
    ("Znf_NHR/GATA", True),
    ("p53-like_TF_DNA-bd", True),
    ("WHTH_DNA-bd_dom", True),
    # Non-DBD names the old regex passed (false positives) — must fail now.
    ("Znf_LIM", False),             # LIM-only zinc fingers, non-TF adaptors
    ("Znf_hrmn_rcpt", False),       # NHR ligand-binding zf
    ("SMAD_dom-like", False),
    ("SMAD_dom_Dwarfin-type", False),
    ("Dwarfin", False),
    ("EGF-like", False),
...
@pytest.mark.parametrize("term,neural,tf", [
    ...
    # 2026-09-06 audit: bare "DNA binding" is a generic MF term (histones,
    # polymerases) and must NOT grant the go_tf bonus.
    ("DNA binding", False, False),
    ("mitochondrial translation", False, False),
    # "head" keyword false positives observed in the corpus — excluded.
    ("head involution", False, False),
    ("specification of segmental identity, head", False, False),
    # Direction-awareness: neuron terms stay neural regardless of sign.
    ("negative regulation of neuron apoptotic process", True, False),
    ("sensory organ development", True, False),
    ("camera-type eye development", True, False),
])
