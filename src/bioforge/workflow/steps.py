...
def score_per_atlas(adata: "ad.AnnData",
    ...
    if cluster_key:
        ...
        for gene in scores:
            ...
            if not mask.any():
                continue
            # best (max |log2FC|) hit across clusters — mirrors the canonical
            # pipeline, which does not settle for the first cluster in order.
            lfcs = result["logfoldchanges"][mask]
            pvals = result["pvals_adj"][mask]
            idx = int(np.argmax(np.abs(np.asarray(lfcs, dtype=float))))
            log2fc = float(lfcs[idx])
            pval = float(pvals[idx])
            if log2fc < min_log2fc or pval > max_pval:
                continue
            # Re-identify v4 gene ids through the bridge so Fincher (v4) and
            # Plass/King (v6) records of the same gene merge downstream.
            gid = v6_of.get(gene) or gene if v6_of else gene
            r = EvidenceRecord(
                gene_id=gid,
...
def add_rnai_stream(records: list[EvidenceRecord], rnai_table: "pd.DataFrame | None" = None,
    ...
    targets = set()
    for t in rnai_targets:
        targets |= _norm(t)

    for r in records:
        ids: set[str] = set()
        if r.gene_name:
            ids |= _norm(r.gene_name)
        ids |= _norm(r.gene_id)
        if bridge is not None:
            v6 = bridge.v4_to_v6(r.gene_id) or r.gene_id
            ids |= _norm(v6)
            ids |= _norm(bridge.v6_to_name(v6)) if v6 else set()
        in_table = bool(ids & targets)
...
def add_correlation_stream(records: list[EvidenceRecord],
    ...
    for r in records:
        name = r.gene_name or ""
        # genes appearing only as tf2 in mmc6 still get scored (the prefilter
        # must not restrict to tf1-only rows — that silently dropped hits).
        hits = relevant[(relevant["tf1"] == name) | (relevant["tf2"] == name)] \
            if name else relevant.iloc[0:0]
        if len(hits) == 0:
            continue
