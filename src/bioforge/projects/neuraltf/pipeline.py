...
# 2026-09-06 isoform-aware matching (audit B2): capture the FULL
# structured ID including the isoform suffix so that
# dd_Smed_v6_9460_0_1 and dd_Smed_v6_9460_0_2 stay distinct. The bare
# dd#### collapse (_RE_DD_SHORT) had been aliasing 446 genes in 206
# isoform groups, cross-crediting RNAi/correlation evidence between
# distinct isoforms.
_RE_DD_STRUCTURED_FULL = re.compile(r"(dd_Smed_v[46]_\d+_\d+_\d+)")
...
class NeuralTFPipeline:
    ...
    def __init__(
        ...
        # 2026-09-06 audit fix (B1): UNCONDITIONED per-gene min-p per atlas.
        # de_pvals above stores only p's from clusters that already passed
        # the global BH q<=0.1 gate — those values are CONDITIONED on a
        # selection event and are not valid Fisher/Stouffer inputs (the
        # meta_analytic consumer previously produced 99.6%
        # "significant" genes from them). The raw min-p below is taken
        # over ALL clusters with positive fold-change, no BH
        # pre-filtering, so the downstream Sidak un-conditioning
        # 1-(1-p)^k_clusters applies to the actual selection family.
        self.de_pvals_raw: dict[str, dict[str, tuple[float, float]]] = {}
    ...
    def _short_id(gene_id: str) -> str | None:
        ...
        2026-09-06 isoform-collision note: 446 genes (206 groups) share
        the same numeric dd part across isoforms (e.g. ..._9460_0_1 vs
        ..._9460_0_2). The short token CANNOT distinguish them — but the
        King tables (mmc5/mmc6) themselves only record bare dd####, so
        short-form matching is the only possible match there. Structured
        v6 IDs are therefore matched ISOFORM-AWARE first
        (_structured_isoform_key); the short token is only used for
        short-form table rows, never to alias distinct v6 isoforms.
        """
    ...
    @staticmethod
    def _structured_isoform_key(gene_id: str) -> str | None:
        """Full structured key ('dd_Smed_v6_9460_0_1') for structured IDs.

        Unlike _short_id, this keeps the isoform suffix so that
        dd_Smed_v6_9460_0_1 and dd_Smed_v6_9460_0_2 remain distinct.
        Returns None for bare dd#### tokens (no isoform information).
        """
        if not gene_id or not isinstance(gene_id, str):
            return None
        m = _RE_DD_STRUCTURED_FULL.match(gene_id.strip())
        if m:
            return m.group(1) if m.group(1) else gene_id.strip()
        return None

    def _all_ids_for_record(self, record: EvidenceRecord) -> set[str]:
        """Collect every string someone might use to name this gene.

        2026-09-06 isoform fix: the structured v6 key (with isoform
        suffix) is now included so a structured-form table row matches
        only the SAME isoform, never a numeric sibling. The short dd####
        token is still included because King mmc5/mmc6 store bare
        dd#### — but it can only ever match short-form rows (see
        _build_rna_target_set, which now keys structured rows by their
        full key and short rows by the short token).
        """
        ids: set[str] = set()
        if record.gene_name:
        ...
        if sid:
            ids.add(sid)
        full = self._structured_isoform_key(record.gene_id)
        if full:
            ids.add(full)
        if self.bridge:
    ...
    def _score_one_atlas(self, adata, atlas_name, result):
        ...
        for (key, cl), qval in zip(gene_cluster_keys, qvals):
            ...
            # UNCONDITIONED min-p (2026-09-06, B1): track the best
            # (minimum) p among ALL positively-fold-changed clusters for
            # this gene — no BH gating. Downstream meta-analysis needs
            # the raw min-p over the gene's actual test family.
            if pos_lfc > 0:
                prev_raw = self.de_pvals_raw.get(key, {}).get(atlas_name, (1.0, 0.0))
                if pval < prev_raw[0]:
                    self.de_pvals_raw.setdefault(key, {})[atlas_name] = (pval, pos_lfc)
    ...
    def _build_rna_target_set(self) -> set[str]:
        ...
        2026-09-06 isoform fix: STRUCTURED rows (dd_Smed_v6_9460_0_1)
        are keyed by their full isoform-specific key — they must never
        alias a numeric sibling isoform through the bare dd#### token.
        SHORT rows (dd####, with or without a parenthetical symbol)
        keep the bare-token key (that is all the information the table
        ...
        for _, row in self.rnai_table.iterrows():
            ...
            if full:
                targets.add(full)
                continue
            # short/gene-symbol forms: bare dd#### token when present
    ...
    def integrate_correlations(self):
        ...
        def normalize(val) -> set[str]:
            ...
            # 2026-09-06 isoform fix: structured rows keep their full
            # isoform-specific key (never collapsed to the bare dd####
            # token, which aliases sibling isoforms); short rows keep
            # the bare token (all the table carries).
            full = self._structured_isoform_key(s)
            if full:
                out.add(full)
            else:
    ...
    def _checkpoint_post_scoring(self) -> None:
        ...
        for gene, per_atlas in self.de_pvals.items():
            ...
            # unconditioned min-p columns (B1): preferred by
            # meta_analytic_pvalue.py as valid Fisher/Stouffer inputs
            raw = self.de_pvals_raw.get(gene, {})
            for atlas in ("fincher", "plass", "cui"):
                if atlas in raw:
                    row[f"{atlas}_p_raw"] = raw[atlas][0]
                    if f"{atlas}_lfc" not in row:
                        row[f"{atlas}_lfc"] = raw[atlas][1]
            de_rows.append(row)
        if de_rows:
