...
$$\text{Score}_{\text{perez_lineage}} = \begin{cases} 1.0 & \text{if TF class } \in \text{Neural Superfamilies (bHLH, Homeobox, POU, C2H2, SOX, FOX, etc.)} \\ 0.5 & \text{if TF class is another confirmed structural TF family} \\ 0.0 & \text{if unclassified or absent from Perez MOESM5 catalog} \end{cases}$$
...
$$\text{Score}_{\text{perez_influence}} = \text{influence_score}_{\text{neuron fate}}$$
