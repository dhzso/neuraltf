...
| `15_method_bumpchart.png` | Candidate Rank Trajectories Across Dirichlet Weighting | Method Concordance | Rank trajectories across Fixed, Centered Dirichlet, and Uniform Dirichlet formulations. |
...
| `24_negative_controls.png` | Specificity Validation Against Negative Control Cohorts | Specificity | Integrated score distribution in neural candidates vs epidermal, gut/muscle, and housekeeping genes. |
...
| `26_permutation_null.png` | Permutation Null vs Observed Candidate Scores | Hypothesis Testing | Empirical null distribution under joint row permutation vs observed top candidate scores ($P < 0.001$). |
...
| `32_perez_influence_comparison.png` | Evidence Score Stratification Across Lineages | Lineage Specificity | Score distribution across single-cell lineage classes from Perez et al. (Mann–Whitney $P < 10^{-15}$). |
