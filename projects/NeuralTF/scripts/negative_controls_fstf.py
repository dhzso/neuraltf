...
W_BY_NAME = {getattr(k, "value", k): float(v) for k, v in DEFAULT_WEIGHTS.items()}
CIRCULAR_STREAMS = ("rnai", "neural_enriched", "neural_specificity", "perez_lineage")


def honest_score(df: pd.DataFrame) -> pd.Series:
    """Label-free weighted score excluding streams that encode the ground truth."""
...
def main() -> int:
    ...
    # 2026-09-19: honest-score ranking (label-free streams only)
    rank["honest_score"] = honest_score(rank)
    rank["honest_rank_pos"] = rank["honest_score"].rank(ascending=False, method="first").astype(int)

    # --- Empirical FPR -------------------------------------------------------
    neg_df = rank[rank["label"] == "negative_control"].copy()
    in_top50 = (neg_df["rank_pos"] <= 50).sum()
    fpr_top50 = in_top50 / max(n_neg, 1)
    in_top50_h = (neg_df["honest_rank_pos"] <= 50).sum()
    fpr_top50_h = in_top50_h / max(n_neg, 1)
    print(f"\n  Empirical FPR (negatives in top 50): {in_top50}/{n_neg} = {fpr_top50:.1%}")
    print(f"  Empirical FPR honest (negatives in top 50): {in_top50_h}/{n_neg} = {fpr_top50_h:.1%}")
    ...
    try:
        ...
        y_score_h = labeled["honest_score"].fillna(0.0).values

        if len(np.unique(y_true)) == 2:
            roc_auc = roc_auc_score(y_true, y_score)
            pr_auc = average_precision_score(y_true, y_score)
            roc_auc_h = roc_auc_score(y_true, y_score_h)
            pr_auc_h = average_precision_score(y_true, y_score_h)
            print(f"  ROC-AUC (neural vs non-neural): {roc_auc:.4f}  honest: {roc_auc_h:.4f}")
            print(f"  PR-AUC  (neural vs non-neural): {pr_auc:.4f}  honest: {pr_auc_h:.4f}")
        else:
            roc_auc = pr_auc = roc_auc_h = pr_auc_h = float("nan")
    ...
    for gid, label_str in NEGATIVE_CONTROLS.items():
        ...
        if len(row_match) == 0:
            bench_rows.append({
                ...
                "honest_rank_pos": "not_in_candidates",
                "integrated_score": float("nan"),
                "honest_score": float("nan"),
                "neural_enriched": float("nan"),
                "rnai": float("nan"),
                "in_top50": False,
                "in_top50_honest": False,
            })
        else:
            r = row_match.iloc[0]
            bench_rows.append({
