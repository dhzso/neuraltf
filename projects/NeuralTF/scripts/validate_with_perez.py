...
def validate_top10():
    ...
    # 2026-09-19: dirichlet_top10_prioritized.csv was a dead path (the
    # centered-prioritization output is dirichlet_centered_top10.csv —
    # see style.load_centered); the CENTERED arm silently skipped on
    # every previous run.
    for name, path in [
        ("FIXED", RESULTS / "top10_neural_tfs_prioritized.csv"),
        ("CENTERED", RESULTS / "dirichlet_centered_top10.csv"),
        ...
        for _, row in df.iterrows():
            ...
            if is_tf:
                ...
                top_target_names = [str(x) for x in tf_targets["Target (gene symbol)"].head(3).tolist() if pd.notna(x)]
                ...
                tf_names = [str(x) for x in targeting[tf_sym_col].unique()[:3] if pd.notna(x)] if tf_sym_col else []
