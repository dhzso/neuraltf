"""Uniform Dirichlet — method-specific base score vs composite for top 10."""
...
def build():
    ...
    for i, (_, row) in enumerate(order.iterrows()):
        ...
        base = row.get("uniform_median_score", np.nan)
    ...
    ax.set_xlim(0, 1.05)
    ax.set_title("Composite score exceeds uniform Dirichlet median for all top-10 candidates",
    ...
    legend_handles = [Patch(facecolor="#555", alpha=0.85, label="Composite (median + bonuses)"),
                      Patch(facecolor="#555", alpha=0.35, label="Uniform median only"),
