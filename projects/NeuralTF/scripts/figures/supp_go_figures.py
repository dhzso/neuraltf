...
def fig_s1_go_landscape():
    ...
    ax.set_ylabel("Candidate", fontsize=7.5)
    ...
    legend_handles = [
        ...
        mpatches.Patch(facecolor=C_A, alpha=0.3, label="Neural GO"),
        mpatches.Patch(facecolor=C_B, alpha=0.3, label="TF GO"),
    ]
    ax.legend(handles=legend_handles, loc="upper right", fontsize=6.5, frameon=True)
    ax.set_title("Gene Ontology (GO) annotation matrix",
...
def fig_s2_go_namespace_and_track():
    ...
    ax1.set_title("Namespace distribution", fontsize=8, pad=4)
    ...
    ax2.bar(x2 - w/2, a_vals, w, color=C_A, alpha=0.85, label="Track A (benchmark)")
    ax2.bar(x2 + w/2, b_vals, w, color=C_B, alpha=0.75, label="Track B (candidate)")
    ax2.set_xticks(x2)
    ax2.set_xticklabels(cats, fontsize=7)
    ax2.set_ylabel("Mean GO terms per candidate", fontsize=7.5)
    ax2.set_title("Annotation count by track", fontsize=8, pad=4)
