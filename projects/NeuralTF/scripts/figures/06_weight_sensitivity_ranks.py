...
def build():
    ...
    ax.axvline(x=30, color=C_HL, lw=0.8, ls="--",
               label="Top-30 boundary (entrant zone)")
    ax.set_xlabel("Rank across 1,000 Dirichlet weight draws (full universe)", fontsize=8)
    ax.set_ylabel("TF candidate (ordered by baseline rank)", fontsize=8)
    ax.invert_yaxis()
    ax.set_title("Candidate rank stability under Bayesian Dirichlet weight perturbation",
