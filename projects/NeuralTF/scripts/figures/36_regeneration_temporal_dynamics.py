...
# Live Cui 2023 atlas summary (generated by projects/NeuralTF/scripts/preprocess_cui.py
# from the raw Cui et al. 2023 h5ad). No baked-in profiles: every value is read from data.
CUI_CSV_CANDIDATES = [
...
def build():
    ...
    cui_path = next((p for p in CUI_CSV_CANDIDATES if p.exists()), None)
    if cui_path is None:
        raise FileNotFoundError(
            "cui_atlas_summary.csv not found — run projects/NeuralTF/scripts/preprocess_cui.py first "
            "(fig 36 no longer carries baked-in expression profiles)."
        )
    ...
    # Every prioritized candidate must have a live Cui profile — no silent fallbacks.
    missing = merged[merged[time_cols].isna().all(axis=1)]
    if len(missing):
        raise ValueError(
            f"No Cui 2023 coverage for {len(missing)} prioritized candidate(s): "
            f"{missing[gid_col].tolist()} — regenerate cui_atlas_summary.csv."
        )
