...
def build():
    ...
    ax.set_yticks(y)
    ax.set_yticklabels([label(rank_all, g) for g in df["gene_id"]], fontsize=6.0)

    ax.axvline(x=0.8, color="#888888", lw=0.8, ls="--", label="Threshold = 80%")
    ax.set_xlabel("Top 10 retention frequency across 1,000 Dirichlet draws", fontsize=7.0)
    ax.set_ylabel("Candidate", fontsize=7.0)
    ax.set_xlim(0, 1.25)
    ax.set_title("Top 10 retention stability (1,000 Dirichlet draws)", fontsize=8.0, pad=6)
    ...
    ax.legend(handles, labels_leg, frameon=False, fontsize=6.0, loc="lower right", bbox_to_anchor=(0.98, 0.04))
