"""Top 10 candidates — horizontal bar chart with scores, domain color-coding, and track labels."""
...
def build():
    ...
    for _, row in top10.iterrows():
        ...
        ortholog = str(row.get("human_ortholog",""))[:15]
        records.append({
            "gene_id": gid, "name": nm, "track": track,
            "integrated": integrated,
            "composite": comp_fixed, "centered": comp_cent, "uniform": comp_unif,
            "proof": proof, "domains": domains, "ortholog": ortholog,
        })
    ...
    # Domain-colored bars (background)
    for i, (_, r) in enumerate(df.iterrows()):
        dc = domain_color(r["domains"])
        ax.barh(y[i], r["composite"], height=0.7, color=dc, alpha=0.25, edgecolor="none")

    # Composite score bars (foreground)
    track_colors = [C_A if r["track"]=="A" else C_B for _, r in df.iterrows()]
    ax.barh(y, df["composite"], height=0.55, color=track_colors, alpha=0.85, edgecolor="white", lw=0.5)

    # Score annotations
    for i, (_, r) in enumerate(df.iterrows()):
        ax.text(r["composite"] + 0.008, y[i], f'{r["composite"]:.3f}',
                fontsize=7, va="center", fontweight="bold", color=track_colors[i])

    # Gene names (color-coded by track)
    ax.set_yticks(y)
    ax.set_yticklabels([f'{r["name"]}' for _, r in df.iterrows()], fontsize=8, fontweight="bold")
    for i, (_, r) in enumerate(df.iterrows()):
    ...
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        ax.text(0.005, y[i] + 0.28, proof_short, fontsize=5.5, va="bottom", color="#666", fontstyle="italic")

    # Domain legend
    legend_patches = [Patch(facecolor=c, alpha=0.4, label=d) for d, c in DOMAIN_COLORS.items()]
    legend_patches.append(Patch(facecolor="#888", alpha=0.4, label="Other"))
    ax.legend(handles=legend_patches, loc="lower right", fontsize=6, frameon=True,
              title="TF domain family", title_fontsize=7)

    # Track legend (manual)
    from matplotlib.lines import Line2D
    track_handles = [Line2D([0],[0], marker="s", color="w", markerfacecolor=C_A, markersize=8, label="Track A (RNAi-validated)"),
                     Line2D([0],[0], marker="s", color="w", markerfacecolor=C_B, markersize=8, label="Track B (novel)")]
    ax2 = ax.twinx()
    ax2.set_yticks([])
    ax2.legend(handles=track_handles, loc="upper right", fontsize=6, frameon=True)
