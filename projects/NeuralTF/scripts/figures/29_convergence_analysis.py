...
def build():
    ...
    if conv_path.exists():
        ...
        ax1.axhline(y=0.95, color="#888888", lw=0.8, linestyle="--", alpha=0.8,
                    label="Stability threshold (0.95)")
        ax1.set_xlabel("Dirichlet draws", fontsize=8)
        ax1.set_ylabel("Rank stability (Spearman $r_s$)", fontsize=8)
        ax1.set_title("Draw convergence (k = 40)",
    ...
    if res_path.exists():
        ...
        ax2.plot(n_perm, minp, color=C_B, lw=1.5, marker="s", markersize=4,
                 label="Resolution: $P_{min} = 1/(n+1)$")
        ax2.axhline(y=0.05, color=C_HL, lw=0.8, linestyle="--", alpha=0.9,
                    label=r"$\alpha = 0.05$")
        ax2.set_xlabel("Permutations ($n$)", fontsize=8)
        ax2.set_ylabel("Minimum detectable $P$-value", fontsize=8)
        ax2.set_title("Permutation test resolution",
                      fontweight="bold", fontsize=8.5, pad=6)
