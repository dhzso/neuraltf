...
def build_csv(top: pd.DataFrame) -> pd.DataFrame:
    # 2026-09-06 audit fix: the previous dict(zip(bridge[v6], bridge[v4]))
    # silently kept an ARBITRARY last row for the 7,619 v6 IDs that map to
    # more than one v4 ID — contradicting the fixed method's
    # map_v6_to_v4 discipline (which blanks ambiguous mappings and flags
    # them). All three methods now share the same mapping.
    ...
    if bridge_path.exists():
        ...
        v4_map = dict(zip(mapping["v6_id"], mapping["v4_id"]))
        flag_map = dict(zip(mapping["v6_id"], mapping["mapping_flag"]))
    else:
        v4_map, flag_map = {}, {}
    ...
    out["v4_mapping_flag"] = out["gene_id_v6"].map(flag_map).fillna("unmapped")
...
def _deterministic_overall_top10(full_rank: pd.DataFrame, n: int = 10) -> pd.DataFrame:
    ...
    out = full_rank.copy()
    tie_cols = [c for c in ("uniform_median_score", "integrated_score", "n_streams")
                if c in out.columns]
