#!/usr/bin/env python
"""Visualize Dirichlet-uniform (alpha=1) prioritization results — enhanced.
...
Generates 7 publication-quality figures from Dirichlet-uniform outputs:
  ...
  2. trackB_top5          - Track B top-5 with domain, uniform median, score details
  3. scatter_fixed_vs_uniform - fixed-weight vs uniform median with correlation stats
  4. scatter_centered_vs_uniform - centered vs uniform median with correlation stats
  3. combined_dual_track  - both tracks with composite, base score, domain annotations
  4. score_shift          - Uniform composite - fixed composite per candidate (with stats)
  5. three_way_comparison - grouped bars: fixed / centered / uniform with stats
...
def fig_trackA_top5() -> None:
    ...
    ax.set_yticks(y)
    labels = [f"{r['gene_name']}  (median={r['uniform_median_score']:.3f}, {_domain_group(r['interpro_domains'])})" 
    ...
    ax.set_title("A · Track A — RNAi-validated (Dirichlet-uniform, α=1)",
...
def fig_trackB_top5() -> None:
    ...
    ax.set_xlabel("Composite score (Uniform median + bonuses)", fontsize=10)
    ax.set_title("B · Track B — Novel candidates (Dirichlet-uniform, α=1)",
                 fontsize=11, fontweight="bold")
    ax.invert_yaxis()
    _style_ax(ax)
...
def fig_scatter_fixed_vs_uniform() -> None:
    ...
    rho, p_spearman = spearmanr(merged["composite_score_fixed"], merged["composite_score_uniform"])
    r_pearson, p_pearson = pearsonr(merged["composite_score_fixed"], merged["composite_score_uniform"])
    ...
    ax.set_title(f"C · Fixed vs Uniform (ρ={rho:.3f}, r={r_pearson:.3f}, p={p_spearman:.2e})",
                 fontsize=11, fontweight="bold")
...
def fig_scatter_centered_vs_uniform() -> None:
    ...
    merged["shift"] = merged["composite_score_uniform"] - merged["composite_score_centered"]
    rho, p_spearman = spearmanr(merged["composite_score_centered"], merged["composite_score_uniform"])
    r_pearson, p_pearson = pearsonr(merged["composite_score_centered"], merged["composite_score_uniform"])
    ...
    ax.set_ylabel("Dirichlet-uniform composite score", fontsize=10)
    ax.set_title(f"D · Centered vs Uniform (ρ={rho:.3f}, r={r_pearson:.3f}, p={p_spearman:.2e})",
                 fontsize=11, fontweight="bold")
...
def fig_score_shift() -> None:
    ...
    ax.axhline(merged["shift"].mean(), color=C_ORANGE, linewidth=1.5, linestyle=":", 
    ...
    ax.set_title(f"F · Score shift (Dirichlet-uniform − fixed) | Mean: {mean_shift:+.4f} | ↑{pos_shifts} ↓{neg_shifts}",
                 fontsize=11, fontweight="bold")
    ax.legend(handles=[
        Patch(facecolor=C_A, label="Track A (RNAi)"),
        Patch(facecolor=C_B, label="Track B (novel)"),
        Line2D([0], [0], color=C_ORANGE, linewidth=1.5, linestyle=":", label=f"Mean = {merged['shift'].mean():+.4f}"),
...
def fig_three_way_comparison() -> None:
    ...
    for i, (col, color) in enumerate(zip(["fixed", "centered", "uniform"], colors)):
        for j, val in enumerate(merged[col]):
            ax.text(j + (i - 1) * width, val + 0.01, f"{val:.3f}",
