#!/usr/bin/env python
"""Cross-method comparison figures for Dirichlet sensitivity analysis.
...
Outputs (PNGs into projects/NeuralTF/figures/):
  - fig_method_score_density.png          (KDE of all 99 candidates, 3 methods)
  - fig_method_rank_correlation.png       (Spearman ρ heatmap)
  - fig_method_score_volatility.png       (per-candidate score range)
  - fig_method_summary.png                (4-panel summary: overlap, tracks, volatility, composite)
...
def _build_all_scores() -> pd.DataFrame:
    ...
    # Get uniform scores from full rank
    uniform_map = dict(zip(full["gene_id_v6"], full["uniform_score"]))
    rank["uniform_score"] = rank["gene_id"].map(uniform_map)

    # Get centered scores from full rank (already loaded above)
    # centered scores are already in rank from above
...
def fig_score_density() -> None:
    ...
    ax.set_title("A · Score density — 3 methods (99 candidates)",
...
def fig_rank_correlation() -> None:
    ...
    for i, ci in enumerate(methods):
        for j, cj in enumerate(methods):
            rho[i, j] = spearmanr(df[ci], df[cj]).correlation

    im = ax.imshow(rho, vmin=0.8, vmax=1.0, cmap="Reds", aspect="equal")
    for i in range(3):
        for j in range(3):
            ax.text(j, i, f"{rho[i, j]:.3f}", ha="center", va="center",
                    fontsize=11, fontweight="bold",
                    color="white" if rho[i, j] > 0.95 else C_BLACK)
    ax.set_xticks(range(3))
    ax.set_yticks(range(3))
    ax.set_xticklabels(labels, rotation=30, ha="right", fontsize=9)
    ax.set_yticklabels(labels, fontsize=9)
    ax.set_title("B · Spearman ρ — method rank correlation",
                 fontsize=11, fontweight="bold")
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="ρ")
    _style_ax(ax)
...
def fig_score_volatility() -> None:
    ...
    colors = [C_A if "known" in row.get("proof_status", "") else C_B for _, row in top20.iterrows()]
    # We don't have proof_status in df, so use track from centered top10
    centered_csv = IN_DIR / "dirichlet_top10_prioritized.csv"
    track_map = {}
    if centered_csv.exists():
        c = pd.read_csv(centered_csv)
...
def fig_method_summary() -> None:
    ...
    ax.set_title("C · Score volatility (top 20)", fontsize=11, fontweight="bold")
    ax.legend(handles=[
        Line2D([0], [0], marker="o", color="w", markerfacecolor=C_A,
               markersize=8, label="Track A"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor=C_B,
               markersize=8, label="Track B")
...
# ---------------------------------------------------------------------------
# Figure 5: 99-neural vs 249-wide comparison
# ---------------------------------------------------------------------------
