...
def _atomic_write_csv(df: pd.DataFrame, path: Path, **kwargs) -> None:
    ...
    Prevents half-written files when the process is killed mid-write.
    """
...
def build_report(top: pd.DataFrame, g0: dict, x1: dict,
                 n_candidates: int | None = None) -> str:
    lines: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    report_path.write_text(
        build_report(top, g0, x1, n_candidates=len(rank)), encoding="utf-8")
