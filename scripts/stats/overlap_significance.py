...
def _strata_of(df: pd.DataFrame, gene_col: str) -> dict[str, set[str]]:
    ...
    the 11,695-gene universe. The old null (10 random genes from N)
    understated the expected overlap by ~40x (E[overlap] ~ 0.0086 under
    the old null vs ~0.375 under the correct within-stratum null for
    tested genes: 5*5/67 + 5*5/11,567), overstating significance by
    ~25-38 orders of magnitude. The corrected null samples 5 genes from
    the tested stratum and 5 from the not-tested stratum, independently
    per method.
    """
    strata = {}
...
def expected_stratified_overlap(n_strata: dict[str, int], N_strata: dict[str, int]) -> float:
    """Expected pairwise overlap under the dual-track within-stratum null.

    Each method draws n_s genes uniformly from stratum s (sizes N_s);
    the expected overlap of two independent draws is
    sum_s n_s^2 / N_s.
    """
    return sum((n_strata[s] ** 2) / max(N_strata[s], 1) for s in n_strata)


def stratified_overlap_p(k_observed: int, n_strata: dict[str, int],
                         N_strata: dict[str, int]) -> float:
    """One-sided Poisson approximation for the stratified overlap null.

    Under the within-stratum null the pairwise overlap count is a sum of
    independent hypergeometric counts; the Poisson approximation with
    lambda = E[overlap] is accurate for these small n_s. Reported
    alongside the exact expected value so consumers can judge the
    approximation.
    """
    lam = expected_stratified_overlap(n_strata, N_strata)
    if lam <= 0:
        return 1.0
    # P(Poisson(lam) >= k)
...
def three_way_stratified_null_p(k3: int, n_strata: dict[str, int],
                                N_strata: dict[str, int]) -> float:
    """3-way overlap p under the dual-track within-stratum null.

    P(a tested-stratum gene is picked by all 3 methods) = prod_s over
    methods of 5/67; the count of 3-way-shared genes is approximately
    Poisson with lambda = sum_s N_s * (n_s/N_s)^3.
    """
    lam = sum(
        N_strata[s] * (n_strata[s] / max(N_strata[s], 1)) ** 3
        for s in n_strata
    )
    if lam <= 0:
        return 1.0
...
def main():
    ...
    N = rank_df["gene_id"].nunique() if len(rank_df) else 11695
    n = 10
    ...
    if "proof_status" in rank_df.columns and len(rank_df):
        n_tested = int((rank_df["proof_status"].astype(str) == "tested").sum())
        N_strata = {"tested": n_tested, "not_tested": max(N - n_tested, 1)}
        n_strata = {"tested": 5, "not_tested": 5}  # dual-track 5+5 design
    else:
        N_strata = {"all": N}
        n_strata = {"all": n}
    ...
    if "tested" in N_strata:
        results["stratified_null"] = {
            ...
            "N_strata": N_strata,
            "n_strata": n_strata,
            "expected_pairwise_overlap": expected_stratified_overlap(n_strata, N_strata),
            "note": (
    ...
    if "tested" in N_strata:
        print(f"  Strata: {n_strata} from {N_strata} "
              f"(expected pairwise overlap under null: "
              f"{expected_stratified_overlap(n_strata, N_strata):.3f})")
    ...
    for i in range(len(method_names)):
        for j in range(len(method_names)):
            ...
            # 2026-09-13: stratification-corrected p (Poisson approx over
            # the within-stratum null); reported alongside the legacy one.
            strat_p = stratified_overlap_p(k, n_strata, N_strata) \
                if "tested" in N_strata else None
    ...
    if len(methods) == 3:
        ...
        k3 = len(three_way)
        # 2026-09-13: dual-track corrected 3-set null (Poisson over the
        # within-stratum null, lambda = sum_s N_s*(n_s/N_s)^3). The legacy
        # uniform-subset Binomial(N, (n/N)^3) is kept for transparency and
        # labelled as an upper bound on significance.
        p3_strat = three_way_stratified_null_p(k3, n_strata, N_strata) \
            if "tested" in N_strata else None
        ...
        results["three_way"] = {
            ...
            "stratified_poisson_p": p3_strat,
            "expected_stratified_overlap": (
                float(sum(N_strata[s] * (n_strata[s] / max(N_strata[s], 1)) ** 3
                          for s in n_strata)) if "tested" in N_strata else None),
            "N_population": N,
        }
        ...
        if p3_strat is not None:
            msg += f", stratified p={p3_strat:.4e}"
        print(msg)
