...
def permutation_resolution(perms_list):
    ...
    are a FAMILY (11,675-gene universe, or the 134-gene neural family
    under the targeted permutation mode). A per-gene alpha of 0.05 is
    never the operative threshold once multiplicity is applied: e.g.
    Bonferroni within the 134-gene family requires p < 3.7e-4, which
    needs n_perm >= ~2,700. The columns below state both the nominal
    and family-corrected capacity.
    """
    results = {}
    family_sizes = {"neural_candidates": 134, "full_universe": 11675}
...
def main():
    ...
    for _, r in res_df.iterrows():
        print(f"  n_perm={int(r['n_perm']):>4}: min p={r['min_detectable_p']:.5f} "
              ...
              f"Bonf-134 family: {r['resolves_bonferroni_neural_candidates']})")
