...
def convergence_from_draws(mat: np.ndarray, sizes, rng, n_rep=20):
    """Split-half convergence of per-gene RANK vectors.

    2026-09-06 audit fix (self-inclusion bias): the previous design
    correlated a size-N PREFIX against the full 1000-draw set that
    CONTAINS it — Spearman(subset, superset) is mechanically inflated
    (the size=1000 row was rho==1.0 by identity, presented as data).
    The unbiased design compares two DISJOINT half-subsets: a size-N
...
def permutation_resolution(perms_list):
    ...
    2026-09-06 note: the gene-level p-values this resolution must serve
    are a FAMILY (the full-universe rank.csv, or the neural family under
    the targeted permutation mode). A per-gene alpha of 0.05 is never
    the operative threshold once multiplicity is applied: e.g.
    Bonferroni within the 143-gene neural family requires p < 3.5e-4,
    which needs n_perm >= ~2,860. The columns below state both the
    nominal and family-corrected capacity. 2026-09-19: family sizes are
    READ FROM THE ARTIFACTS (previously stale hard-codes 134/11,675
    understated the Bonferroni capacity by ~7%/0.2%).
    """
    results = {}
    family_sizes = {}
    ...
    if rank_path.exists():
        family_sizes["full_universe"] = int(
            pd.read_csv(rank_path, usecols=["gene_id"])["gene_id"].nunique())
    if neural_path.exists():
        family_sizes["neural_candidates"] = int(
            pd.read_csv(neural_path, usecols=["gene_id"])["gene_id"].nunique())
    if not family_sizes:
        family_sizes = {"neural_candidates": 143, "full_universe": 11696}
    for n_perm in perms_list:
        ...
        for fam_name, fam_n in family_sizes.items():
            alpha_bonf = 0.05 / fam_n
            row[f"resolves_bonferroni_{fam_name}"] = bool(p_floor < alpha_bonf)
        results[n_perm] = row
    return results
...
def main():
    ...
    # --- Panel 1: convergence from the real draws (split-half design) ---
    # max size = half the budget (split-half compares two disjoint halves)
    ...
    print(f"\nConvergence (split-half Spearman of per-gene RANK vectors, "
          f"disjoint halves):")
