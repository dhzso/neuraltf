...
def adjusted_min_p(p_min: float, k_eff: int) -> float:
    ...
    2026-09-19: the result is clamped to <= 1-1e-16. For p_min close to
    1 and modest k_eff, 1-(1-p)^k rounds to EXACTLY 1.0 in floating
    point; stats.norm.isf(1.0) then returns -inf, poisoning the Stouffer
    z (and log(1.0)=0 silently zeros the Fisher contribution). The clamp
    keeps both combiners finite for every input.
    """
    p_ceiling = 1.0 - 1e-16
    if k_eff <= 1:
