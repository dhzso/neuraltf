#!/usr/bin/env python
"""Weight-perturbation intervals for integrated evidence scores.

2026-09-06 audit relabel (was "Bootstrap confidence intervals"): the
intervals produced here are WEIGHT-SENSITIVITY BANDS, not bootstrap
confidence intervals on the score's sampling distribution. Only the
Dirichlet weight vector varies; the per-gene stream scores, cells, and
atlases are fixed. No data-generating unit is resampled, so the interval
has no frequency interpretation for the score itself. Consumers (figure
25, reports) must present it as "score range under weight uncertainty".

Additional honesty constraints:
- The same draw shifts every gene JOINTLY, so per-gene bands cannot
  support cross-gene claims ("A's score exceeds B's"). Rank uncertainty
  is the relevant cross-gene quantity and lives in the Dirichlet
  full-rank files / weight-sensitivity outputs.
- Column names keep the historical ci_95_* spellings for figure-25
  compatibility, but the JSON-free CSV now carries an interpretation
  note printed at run time.
...
Outputs:
  results/bootstrap_scores_ci.csv  — per-gene mean + weight-perturbation band
  (consumed by figure 39, the score-uncertainty forest; the retired
  figure 25 was the historical consumer)
...
# canonical stream addition/rename would silently shrink these bands).
import os
...
def weight_bootstrap_cis(df: pd.DataFrame, n_boot: int, seed: int) -> pd.DataFrame:
    ...
    # 2026-09-19: name-keyed weight lookup (the previous positional
    # W_DEFAULT[:len(stream_cols)] slice silently mis-aligned every
    # weight after any MIDDLE stream absent from rank.csv — the same bug
    # class fixed in score_shuffling_permutation.py on 2026-09-13).
    W = np.array([W_BY_NAME[s] for s in stream_cols])
    W = W / W.sum()
...
def main():
    ...
    print(f"\nTop-10 by integrated score — mean and weight-perturbation "
          f"band (NOT a sampling-distribution CI; same-draw bands shift "
          f"jointly, so overlapping bands imply nothing about rank order):")
    for _, row in out.head(10).iterrows():
        label = row.get("gene_name", row["gene_id"])
        print(f"  {str(label):>14}  mean={row[score_col]:.4f}  "
              f"band=[{row[ci_lo_col]:.4f}, {row[ci_hi_col]:.4f}]")
