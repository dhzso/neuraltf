...
# 2026-09-19: streams directly encoding the ground-truth labels — every
# phenotype-confirmed gene carries rnai=1.0 INSIDE the integrated score,
# so a phenotype arm built on the full score is partially circular.
...
def honest_score(df: pd.DataFrame) -> pd.Series:
    ...
    (identical recomputation to effect_sizes.py / precision_recall.py)."""
...
def main():
    ...
    # producing "enrichment 0.0x, p=1.0" (62/68 positives live in
    ...
    # ---- 2026-09-19: HONEST phenotype arm (label-free score) -----------
    # Every phenotype-confirmed gene carries rnai=1.0 INSIDE the
    # integrated score, so the deciles above are partially circular.
    # This arm recomputes deciles from the label-independent honest
    # score (excluding rnai/neural_enriched/neural_specificity/perez_
    # lineage), providing the genuinely held-out discrimination test.
    df["honest_score"] = honest_score(df)
    df["honest_decile"] = pd.qcut(
        df["honest_score"].rank(method="first"), q=args.n_bins, labels=False)
    df["honest_decile"] = args.n_bins - 1 - df["honest_decile"]
    honest_top = df[df["honest_decile"] == 0]
    k_top_h = int(honest_top[conf_col].sum())
    n_top_h = int(len(honest_top))
    ci_lo_h = float(_st.beta.ppf(0.025, k_top_h, n_top_h - k_top_h + 1)) if k_top_h > 0 else 0.0
    ci_hi_h = float(_st.beta.ppf(0.975, k_top_h + 1, n_top_h - k_top_h)) if k_top_h < n_top_h else 1.0
    enrich_h = (k_top_h / n_top_h) / prev_conf if prev_conf > 0 else float("nan")
    p_h = float(_st.binomtest(k_top_h, n_top_h, prev_conf,
                              alternative="greater").pvalue) \
        if hasattr(_st, "binomtest") and prev_conf > 0 else float("nan")
    per_decile_pos_honest = [
        int(df[df["honest_decile"] == d][conf_col].sum())
        for d in sorted(df["honest_decile"].unique())
    ]
    print(f"[phenotype_confirmed honest_score] top-decile {k_top_h}/{n_top_h}, "
          f"enrichment={enrich_h:.1f}x, p={p_h:.3e}")
    ...
    results = {
        ...
        "phenotype_confirmed": {
            ...
            "circularity_note": (
                "Deciles defined by integrated_score which CONTAINS rnai=1.0 "
                "for every phenotype-confirmed gene — partially circular. "
                "Use phenotype_confirmed_honest below for the held-out test."
            ),
        },
        "phenotype_confirmed_honest": {
            "score_used": "honest_score (label-free streams only)",
            "n_positives": n_conf,
            "prevalence": float(prev_conf),
            "top_decile_enrichment": {
                "fold_enrichment": float(enrich_h),
                "top_positive_rate": float(k_top_h / n_top_h) if n_top_h else 0.0,
                "rate_ci95": [ci_lo_h, ci_hi_h],
                "p_one_sided_binomial": None if p_h != p_h else float(p_h),
                "n_positives_top_decile": k_top_h,
                "n_candidates_top_decile": n_top_h,
            },
            "positives_per_decile": per_decile_pos_honest,
        },
        "discrimination_note": (
