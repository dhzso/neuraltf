...
(rank_neural.csv, 134 genes) so n=1000 draws resolve p ~ 1e-3 with honest
BH WITHIN that family (Bonferroni alpha = 0.05/134 = 3.7e-4 needs
n>=2,700 for full family control; n=1000 resolves BH-q at the 134-gene
...
def main():
    ...
    pvals = {}
    # family = candidate_ids (the 134-gene neural set in --candidates
    # mode; the full universe otherwise). out_df MUST be built from this
    # family, not real_scores — building from real_scores silently
    # exported all 11,675 genes with p=1.0 (no null draws) in --candidates
    ...
    # 2026-09-06: family multiplicity. The p-values are a family; per-gene
    # nominal alpha was never the operative threshold. BH is applied
    # across the exported family (134 genes in --candidates mode; 11,675
    # in full mode), and the resolution floor 1/(n+1) is stated against it.
    from statsmodels.stats.multitest import multipletests
    qvals = multipletests(out_df["empirical_p"].values, alpha=0.05,
                          method="fdr_bh")[1]
    out_df["q_bh_family"] = qvals
    out_df["significant_fdr_05"] = qvals < 0.05
    out_df.to_csv(out_path, index=False)
    p_floor = 1.0 / (args.n_perm + 1)
    fam_n = len(out_df)

    testable = out_df[~out_df["untestable_by_permutation"]]
    sig = testable[testable["significant_fdr_05"]]
    print(f"\nSignificant after BH-FDR q<0.05 within the {fam_n}-gene family: "
          f"{len(sig)} / {len(testable)} testable "
