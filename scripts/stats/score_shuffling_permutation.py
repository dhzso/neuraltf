...
def main():
    ...
    # BH-FDR across the full 11,675-gene family (2026-09-06): the
    # exported per-gene p-values are a family; uncorrected p<0.05
    # claims were invalid (~584 false positives expected under H0).
    ...
    # Family max-statistic (2026-09-06 relabel): the global contrast is
    # the max score over the ENTIRE universe against the null's per-
    # permutation max — NOT a "top-10" statistic (the old name mislabeled
    # a universe-level test).
    universe_real_max = np.max(real_scores)
    global_p = (np.sum(np.array(top10_null_max) >= universe_real_max) + 1) / (args.n_perm + 1)
    ...
    print(f"Global max-statistic (universe max vs null max): p={global_p:.4e}")
