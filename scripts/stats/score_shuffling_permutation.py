...
1. JOINT whole-row permutation. The previous null shuffled each stream
   ...
   significance (~1,200/11,696 at p<0.05 against ~585 expected).
...
2. Family multiplicity. 11,696 per-gene p-values are now BH-corrected
...
# 2026-09-19: import the single-source-of-truth weights (the previous
# hand-copied vector carried a "must match exactly" comment but no check;
...
def main():
    ...
    # BH-FDR across the target family (143 genes in --candidates mode, 11,696 in full mode)
    ...
    if args.candidates:
        n_expected_h0 = max(1, int(0.05 * len(pvals)))
        print(
            f"\nCIRCULARITY DISCLOSURE (2026-09-13, updated 2026-09-21):\n"
            f"The --candidates family is DEFINED as neural_enriched>0 OR rnai>0,\n"
            f"and the tested statistic is the FULL score including those streams —\n"
            f"family members are extreme BY CONSTRUCTION.  Expected ~{n_expected_h0}\n"
            f"nominal p<0.05 under a true H0, but observed {n_sig_05}/{len(pvals)}.\n"
            f"This massive inflation is consistent with by-construction extremeness,\n"
            f"not evidence of neural function.  The per-gene p/q values in this file\n"
            f"therefore quantify POPULATION-RELATIVE extremeness only.\n"
            f"The label-free evaluation lives in precision_recall.py\n"
            f"(honest/king_free arms) and negative_controls.py."
        )
